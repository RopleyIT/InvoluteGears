        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                if(args[0] == "-h")
                {
                    ...
                    Console.WriteLine(
                        "-p [tooth count] [profile shift] [tolerance] [angle] [module] [backlash] [stubbing]\r\n"
                            ...
                            + "\tstubbing is in thousandths of the module");
                    ...
                    Console.WriteLine("-s gear1Teeth gear2Teeth -- contact ratios for various addendum/dedendum stubbings");
                ...
                if (args[0] == "-s")
                {
                    if (args.Length != 3 ||
                        !int.TryParse(args[1], out int gteeth1) || !int.TryParse(args[2], out int gteeth2))
                    {
                        Console.WriteLine("Usage: InvoluteConsole -s 12 48\r\n\tWhere 12 and 48 are teeth on mating gears");
                        return;
                    }
                    GearParameters gear1;
                    GearParameters gear2;
                    for (double s = 0; s <= 0.3; s += 0.05)
                    {
                        gear1 = new GearParameters(gteeth1, 1.0, Math.PI / 9, 0.0, 0.0, 0.0, s);
                        gear2 = new GearParameters(gteeth2, 1.0, Math.PI / 9, 0.0, 0.0, 0.0, s);
                        double undercutContactRatio = gear1.ContactRatioWith(gear2);
                        double minGap = gear1.ToothGapAtUndercut;
                        Console.WriteLine($"{s}\t{gteeth1}\t{gteeth2}\t{minGap:F3}\t{undercutContactRatio:F3}");
                    }
                    return;
                }
                if (args[0] == "-p")
                {
                    if (args.Length != 8
                        ...
                        || !int.TryParse(args[7], out int stubbing))
                    {
                        Console.WriteLine("Usage: InvoluteConsole -p [tooth count] [profile shift] [tolerance] [angle] [module] [backlash] [stubbing]\r\n"
                            + "\twhere tooth count is digits\r\n"
                            + "\tprofile shift is in thousandths of the module\r\n"
                            + "\ttolerance is in 1000ths of mm\r\n"
                            + "\tangle is pressure angle in 10ths of a degree\r\n"
                    ...
                    Image img = p.PlotGraphs(new List<IEnumerable<PointF>> {   dedendCircle, baseCircle, pitchCircle, addendCircle,
                        ...
                        2048, 2048);
                    img.Save($"t{gear.ToothCount}p{shift}a{pressureAngle}m{module}b{backlash}s{stubbing}.bmp", ImageFormat.Bmp);
                }
            }
        }
    }
